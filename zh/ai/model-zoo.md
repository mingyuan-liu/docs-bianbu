...
mkdir -p spacemit_ai && cd spacemit_ai
...
解压后默认模型路径为：`~/.cache/models/vad/silero/silero_vad.onnx`。
...
性能数据通常随模型、线程数、平台（K3/KX/PC）与编译选项变化较大。以下为当前 `model_zoo` 各组件 README 中提供的阶段性实测数据汇总（均基于 K3 平台，具体测试方法见各组件 README）。
...
> 说明：以下性能数据是基于 K3 平台按照组件 README 中步骤的实测数据，持续优化中。
